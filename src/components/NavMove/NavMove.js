...
class NavMove extends Component {
  constructor() {
    super();
    this.state = {
      hover: false,
  ...
  componentDidMount() {
    window.addEventListener("scroll", this.moveNavFunc);
  }

  // 스크롤 값에 따라 nav hide 온오프 //
  movingPoint = 100;
  moveNavFunc = (event) => {
    const scrolling = ("scroll", event.srcElement.scrollingElement.scrollTop);
    if (scrolling > this.movingPoint) {
      this.setState({ navMoving: true });
    } else {
      this.setState({ navMoving: false });
    }
  };
  ...
  render() {
    ...
    return (
      <div className="coverMe">
        <div
          className={this.state.navMoving ? "nav navOn" : "nav navOff"}
          onMouseLeave={this.menu_leave}
        >
          <div
            className={
              this.state.navMoving
                ? "header_top_container topOn"
                : "header_top_container topOff"
            }
          >
          ...
          <div
            className={
              this.state.navMoving
                ? "header_bottom_container bottomOn"
                : "header_bottom_container bottomOff"
            }
          >
            <div
              className={this.state.navMoving ? "bottomLogo logoOn" : "logoOff"}
            ...
            <div
              className={
                this.state.hover
                  ? "coverMeHide coverMeHideOn"
                  : "coverMeHide coverMeHideOff"
              }
            >
...
export default withRouter(NavMove);
